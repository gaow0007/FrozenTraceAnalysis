...
arch_list = ['yolo'] 
max_list = [16*4]
local_list = [x * 4 for x in [4, 6, 8, 12, 16]]
...
def fetch_time(stats): 
    ...
    return step_time, sync_time, stats['abs_time'] if 'abs_time' in stats else 1e-3
...
for app in [('VOC-yolo', 'yolo')]: 
    ...
    rewrite_stats = np.load('/mnt/lustre/wgao/workspace/adaptdl/benchmark/models/yolov3/accelerate/model_{}_rewrite_True.npy'.format(app[0].split('-')[1]), allow_pickle=True).tolist() 
    base_stats = np.load('/mnt/lustre/wgao/workspace/adaptdl/benchmark/models/yolov3/accelerate/model_{}_rewrite_False.npy'.format(app[0].split('-')[1]), allow_pickle=True).tolist() 
    ...
    if True: 
        ...
        if True: 
            ...
            for (batch, frozen) in arch_info[arch]: 
                ...
                # import pdb; pdb.set_trace() 
                print('app {}, batch {}, frozen {}, rewrite accelerate {}'.format(app, batch, frozen, (step_time ) / (rewrite_step_time )))
