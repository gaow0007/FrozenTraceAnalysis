...
def _predict_memory(params, batch_size, activation_size, param_size):
    ...
    # return params.alpha_m + ((params.beta_a * activation_size * batch_size) ** params.gamma  + (params.beta_p * param_size)**params.gamma) ** (1 / params.gamma)
    return params.alpha_m + params.beta_a * (activation_size ** params.gamma) * batch_size
...
def _mem_obj_fn(params, batch_size, activation_size, param_size, memory):
    ...
    reg1 = 1e-3 * (params.gamma - 1) ** 2 + 1e-3 * params.beta_p ** 2
    return err1 + reg1
...
for arch_info in [('cifar10-VGG19', 'VGG19'), ('cifar10-ResNet18', 'ResNet18')]: # , ('cifar10-GoogLeNet', 'GoogLeNet'), ('cifar10-ResNet18', 'ResNet18'), ('cifar10-VGG19', 'VGG19'), ('cifar10-ResNet50', 'ResNet50'), ('cifar10-MobileNetV2', 'MobileNetV2'), ('cifar10-GoogLeNet', 'GoogLeNet')]:
    info_dict = dict() 
    ...
    if True: 
        ...
        for frozen_layer in frozen_list[:4]: 
            train_list = [(frozen_layer, batch) for batch in batch_list]
            # data preparation 
            ...
            batch_limit = predict_max_local_bz(params, activation_info[frozen], param_info[frozen], memory_stats['total_memory'], (256, 50000))
            print('-'*50)
            print('frozen layer {}, predict batch limit {}'.format(frozen_layer, batch_limit))
            for batch in batch_list: 
                potential_max_batch = int(memory_stats['total_memory'] / memory_stats[frozen][batch] * batch)
                print('base batch {}, predict potential batch limit {}'.format(batch, potential_max_batch))
        ...
        train_list = [(frozen, batch) for frozen, batch in itertools.product(frozen_list, batch_list[-1:])]
        # print(train_list)
        # import pdb; pdb.set_trace() 
        train_batch_size = list() 
        train_activation_size = list() 
        ...
        # for frozen_layer in frozen_list[:2]: 
        ...
        if True: 
            ...
            for batch in batch_list[-1:]: 
                ...
                for frozen_layer in frozen_list:  
                    ...
                    estimate_memory = _predict_memory(params, batch, activation_info[frozen_layer] * activation_info[-1], param_info[frozen_layer] * param_info[-1]) 
        ...
        if True: 
            ...
            for frozen_layer in frozen_list[:4]: 
                ...
                for batch in batch_list: 
                    ...
                    estimate_memory = _predict_memory(params, batch, activation_info[frozen_layer] * activation_info[-1], param_info[frozen_layer] * param_info[-1]) 
                    estimate_list.append(estimate_memory) 
                plt.plot(batch_list, memory_list, marker='*', label='gt-{}'.format(frozen_layer))
                # plt.plot(batch_list, estimate_list, color='r', label='estimate') 
            plt.legend() 
            plt.title(arch_info[1])
            plt.xlabel('batch')
