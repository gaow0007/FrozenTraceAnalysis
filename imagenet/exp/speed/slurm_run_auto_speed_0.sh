...
do 
    ...
    do
        ...
        then 
            # echo ${placement[@]}
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 -u main_speed.py --arch=$model --batch-size=$batch_size --placement=$placement_str --max_bs=$max_bs --lr=0.08 &
        else
            # echo "$i"_"$node_id"
            ADAPTDL_REPLICA_RANK=$i srun --nodes=1 --gres=gpu:1 --ntasks=1 -w $node_id python3 -u main_speed.py --arch=$model --batch-size=$batch_size --placement=$placement_str --max_bs=$max_bs --lr=0.08
        fi
